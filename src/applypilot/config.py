...
def get_chrome_path() -> str:
    """Auto-detect a supported Chromium-based browser executable.
    ...
    raise FileNotFoundError(
        "No supported Chromium-based browser found. Install Chrome/Arc/Brave or set CHROME_PATH."
    )
...
def get_apply_backend() -> str:
    """Return the configured auto-apply backend name."""
    backend = (os.environ.get("APPLYPILOT_APPLY_BACKEND") or "claude").strip().lower()
    return backend or "claude"


def get_apply_agent_command() -> str | None:
    """Return the configured command for the generic apply backend."""
    command = (os.environ.get("APPLYPILOT_AGENT_COMMAND") or "").strip()
    return command or None


def has_apply_agent_backend() -> bool:
    """Check whether the configured auto-apply backend is available."""
    backend = get_apply_backend()
    if backend == "command":
        return get_apply_agent_command() is not None
    if backend == "claude":
        return shutil.which("claude") is not None
    return False


def get_apply_backend_requirement() -> str:
    """Describe what is missing for the configured auto-apply backend."""
    backend = get_apply_backend()
    if backend == "command":
        return "Set APPLYPILOT_AGENT_COMMAND for the command apply backend"
    if backend == "claude":
        return "Claude Code CLI — install from [bold]https://claude.ai/code[/bold]"
    return (
        "Unsupported APPLYPILOT_APPLY_BACKEND="
        f"'{backend}' — choose 'claude' or 'command'"
    )
...
def get_tier() -> int:
    ...
    Tier 3 (Full Auto-Apply):       + apply backend + Chrome
    """
    load_env()
    ...
    has_apply_backend = has_apply_agent_backend()
    try:
        get_chrome_path()
        has_chrome = True
...
def check_tier(required: int, feature: str) -> None:
    ...
    if required >= 3:
        if not has_apply_agent_backend():
            missing.append(get_apply_backend_requirement())
        try:
            get_chrome_path()
        except FileNotFoundError:
            missing.append("Chromium-based browser — install Chrome/Arc/Brave or set CHROME_PATH")
