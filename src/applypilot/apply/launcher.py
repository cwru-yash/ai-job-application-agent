"""Apply orchestration: acquire jobs, spawn apply agent sessions, track results.
...
def run_job(
    ...
    """Run one job application through the selected apply agent backend.
    ...
    try:
        ...
        if agent_result.skipped:
            return "skipped", agent_result.duration_ms
        ...
        job_log = config.LOG_DIR / (
            f"agent_{ts}_w{worker_id}_{agent.backend}_{job.get('site', 'unknown')[:20]}.txt"
        )
        job_log.write_text(output, encoding="utf-8")

        if agent_result.stats:
            cost = agent_result.stats.get("cost_usd", 0)
            ws = get_state(worker_id)
...
def worker_loop(worker_id: int = 0, limit: int = 1,
                ...
                agent_command: str | None = None) -> tuple[int, int]:
    ...
    Args:
        ...
        agent_backend: Apply backend name.
        agent_command: Command used by the command backend.
    ...
    empty_polls = 0
    port = BASE_CDP_PORT + worker_id
    agent = build_apply_agent(backend=agent_backend, command=agent_command)
...
def main(limit: int = 1, target_url: str | None = None,
         ...
         agent_backend: str | None = None,
         agent_command: str | None = None) -> None:
